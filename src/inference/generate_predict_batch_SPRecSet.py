...
def format_prompt(instruction, input_text):
    ...
    if input_text.strip():
        ...
        preamble_no_input = "Below is an instruction that describes a task. Write a response that appropriately completes the request."
        return f"""{preamble_no_input}
...
def generate_predictions_batch(model, tokenizer, dataset, batch_size=8, max_new_tokens=50):
    ...
    for i in tqdm(range(0, len(dataset), batch_size), desc="Generating Predictions"):
        ...
        s = outputs.sequences
        decoded_outputs = tokenizer.batch_decode(s, skip_special_tokens=True)

        for j, decoded_text in enumerate(decoded_outputs):
            if 'Response:\n' in decoded_text:
                response = decoded_text.split('Response:\n')[-1].strip()
            else:
                response = decoded_text[len(prompts[j]):].strip()
...
def generate_predictions(config):
    ...
    # Load Tokenizer
    tokenizer = AutoTokenizer.from_pretrained(base_model, padding_side="left", local_files_only=True, torch_dtype=torch.float16)
