...
def generate_candidates_beam_batch(model, tokenizer, prompts, config, trie=None):
    ...
    def prefix_allowed_tokens_fn(batch_id, input_ids):
        ...
        response_only_prefix = input_ids[response_start:]
...
def process_data(train_path, valid_path, model, tokenizer, config):
    ...
    expected_train_size = config.get("train_size", 1024)
    expected_valid_size = config.get("valid_size", 256)


    chunk_size = 100 
    existing_chunks = glob.glob(os.path.join(save_dir, "candidates_chunk_*.json"))


    if existing_chunks:
        # 找出最大的 chunk 編號
        chunk_numbers = [int(re.findall(r'\d+', os.path.basename(f))[0]) for f in existing_chunks]
        next_file_counter = max(chunk_numbers) + 1
        # 計算應該跳過的資料筆數
        start_index = next_file_counter * chunk_size
        print(f"Detected {len(existing_chunks)} existing chunks. Resuming from index {start_index} (Chunk {next_file_counter})...")
    else:
        next_file_counter = 0
        start_index = 0
        print("No existing chunks found. Starting from the beginning.")

    if start_index >= len(full_data):
        print("All data seems to be processed already.")
    else:
        valid_books = list(name2genre.keys())
        trie = Trie(tokenizer.eos_token_id)
        for name in valid_books:
        ...
        for i in tqdm(range(start_index, len(full_data), batch_size), desc="Generating Negatives"):
            ...
            for idx, (d, prompt) in enumerate(zip(batch, prompts)):
                ...
                # 滿 100 筆存一次檔
                if len(temp_results) >= chunk_size:
                    chunk_path = os.path.join(save_dir, f"candidates_chunk_{file_counter}.json")
                    safe_write_json(chunk_path, temp_results)
                    print(f"\nSaved chunk {file_counter}")
                    temp_results = []
                    file_counter += 1
    ...
    # --- 4. 自動整合所有檔案 ---
    print("\nStarting to merge all chunks into final files...")
    all_merged_results = []
    
    # 重新掃描所有 chunk (包含舊有的和剛生成的)
    final_chunk_files = sorted(
        glob.glob(os.path.join(save_dir, "candidates_chunk_*.json")),
        key=lambda x: int(re.findall(r'\d+', os.path.basename(x))[0])
    )

    for cf in final_chunk_files:
        all_merged_results.extend(safe_load_json(cf))

    # 依照原定數量切分
    train_results = all_merged_results[:expected_train_size]
    valid_results = all_merged_results[expected_train_size : expected_train_size + expected_valid_size]

    safe_write_json(os.path.join(save_dir, "train.json"), train_results)
    safe_write_json(os.path.join(save_dir, "valid.json"), valid_results)

    print(f"Successfully merged {len(all_merged_results)} samples.")
    print(f"Final files: \n - {os.path.join(save_dir, 'train.json')}\n - {os.path.join(save_dir, 'valid.json')}")
